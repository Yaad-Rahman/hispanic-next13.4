...
export default async function Board() {
  ...
  const categories =
    memberCategories.payload?.content &&
    memberCategories.payload.content.length > 0 &&
    memberCategories.payload.content.map((category) => ({
      id: category.id,
      name: category.name,
    }));
  ...
  const data = dataPromises && (await Promise.all(dataPromises));

  const filteredData =
    data && (data.filter((item) => item !== null) as BoardPageProps['data']);
